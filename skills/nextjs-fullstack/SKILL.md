...
description: Next.js App Router expert guidance. Use when building, debugging, or architecting Next.js applications — routing, Server Components, Server Actions, Cache Components (PPR/use cache), layouts, middleware/proxy, data fetching, rendering strategies, Turbopack bundler, and deployment on Vercel.
metadata:
  priority: 5
  docs:
...
validate:
  -
    ...
    severity: error
    upgradeToSkill: nextjs-fullstack
  ...
  -
    ...
    message: 'next/head is Pages Router — use export const metadata or generateMetadata() in App Router. Run Skill(nextjs-fullstack) for metadata API guidance.'
    severity: error
    upgradeToSkill: nextjs-fullstack
...
- Key features: instant HMR, file system caching, multi-env builds
- Configuration in Next.js 16 (top-level `turbopack` key)
- CSS, Sass, PostCSS, CSS Modules handling
- Tree shaking and bundle analysis
- Migrating webpack loaders to Turbopack equivalents
- Production build diagnostics and performance profiling
- Turbopack vs Webpack comparison

## Cache Components (PPR)

See [cache-components.md](./cache-components.md) for:
- Enabling `cacheComponents: true` (replaces `experimental.ppr`)
- Three content types: static, cached (`use cache`), dynamic (Suspense)
- `cacheLife()` built-in profiles and inline configuration
- `cacheTag()` and `updateTag()` / `revalidateTag()` for invalidation
- Runtime data constraint and the `use cache: private` exception
